...
from features import get_atom_feature_dims, get_bond_feature_dims
...
class AtomEncoder(nn.Module):
    ...
    def init_parameters(self):
        for i, dim in enumerate(self.full_atom_feature_dims):
            emb = nn.Embedding(dim, self.emb_dim)
...
class GNN(nn.Module):
    ...
    def reset_parameters(self):
        self.node_encoder.init_parameters()
...
class GNN_DDI(nn.Module):
    def __init__(self, args, num_tasks=1, molecule_model=None):
    ...
    def reset_parameters(self):
        self.molecule_model.reset_parameters()
    ...
    def forward(self, batch):
        mol1_data, mol2_data = batch['molecule1_data'], batch['molecule2_data']
        mol1_graph_representation = self.get_graph_representation(mol1_data)
        mol2_graph_representation = self.get_graph_representation(mol2_data)

        link_representation = torch.cat([mol1_graph_representation, mol2_graph_representation], dim=-1)
        output = self.graph_pred_linear(link_representation)
